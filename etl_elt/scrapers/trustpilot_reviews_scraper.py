...
class TrustpilotReviewsScraper:
    ...
    def scrape_all_reviews(
        ...
        logger.info(f"Démarrage du scraping des avis depuis: {company_url}")
        ...
        while True:
            ...
            if max_reviews and len(reviews) >= max_reviews:
                logger.info(f"Limite d'avis atteinte: {max_reviews}")
                break
            ...
            try:
                ...
                logger.info(f"Trouvé {len(page_reviews)} avis sur la page {page}. Total: {len(reviews)}")
                
                # Page suivante?
                if not self._has_next_page(soup):
                    logger.info("Plus de pages disponibles")
                    break
        ...
        logger.info(f"Total d'avis récupérés: {len(reviews)}")
        self.reviews = reviews
        return reviews
    
    def _scrape_with_filters(self, company_url: str, max_reviews: Optional[int] = None) -> List[Dict]:
        """Scrape avec filtres"""
        logger.info("Utilisation de la stratégie multi-filtres pour contourner les limites de pagination")
        ...
        for stars in [5, 4, 3, 2, 1]:
            ...
            logger.info(f"\n{'='*60}")
            logger.info(f"Scraping des avis {stars} étoiles")
            logger.info(f"{'='*60}")
            ...
            while True:
                ...
                try:
                    ...
                    logger.info(f"Trouvé {len(page_reviews)} avis ({new_reviews} nouveaux). Total: {len(all_reviews)}")
                    
                    if not self._has_next_page(soup):
                        logger.info(f"Plus de pages pour les avis {stars} étoiles")
                        break
        ...
        logger.info(f"\n{'='*60}")
        logger.info(f"Total d'avis uniques récupérés: {len(all_reviews)}")
        logger.info(f"{'='*60}\n")
    ...
    def _extract_reviews_from_page(self, soup: BeautifulSoup) -> List[Dict]:
        ...
        for card in review_cards:
            try:
                ...
                logger.error(f"Erreur lors de l'extraction de l'avis: {e}")
                continue
    ...
    def _extract_review_data(self, card) -> Optional[Dict]:
        ...
        try:
            ...
            # Score
            if 'stars' in review:
                review['score'] = review['stars'] / 5.0 * 10
            ...
            if content_elem:
                ...
                review['comment_text'] = content_elem.get_text(strip=True)
            else:
                review['content'] = ''
                review['comment_text'] = ''
            ...
            if date_elem:
                review['date_absolute'] = date_elem.get('datetime', '')
                review['date'] = date_elem.get('datetime', '')
                review['date_relative'] = date_elem.get_text(strip=True)
                review['date_text'] = date_elem.get_text(strip=True)
            else:
            ...
            if author_elem:
                ...
                review['reviewer_name'] = author_elem.get_text(strip=True)
            else:
                review['author_name'] = ''
            ...
            review['company_replied'] = reply_elem is not None
            ...
            if reply_elem:
                ...
                if reply_content:
                    ...
                    review['company_reply_text'] = reply_content.get_text(strip=True)
                else:
            ...
            logger.error(f"Erreur lors de l'extraction des données de l'avis: {e}")
            return None
    ...
    def get_company_stats(self, company_url: str) -> Dict:
        """Stats entreprise"""
        try:
            logger.info(f"Récupération des stats de l'entreprise: {company_url}")
            ...
            logger.error(f"Erreur lors de la récupération des stats de l'entreprise: {e}")
            return {}
    ...
    def save_to_json(self, reviews: List[Dict], filename: str):
        ...
        try:
            ...
            logger.info(f"Avis sauvegardés dans {filename}")
        except Exception as e:
    ...
    def save_to_jsonl(self, reviews: List[Dict], filename: str):
        ...
        try:
            ...
            logger.error(f"Erreur lors de la sauvegarde en JSONL: {e}")
